...
const TransferDetailBox: React.FC<Props> = ({ transfer, onClose, onUpdate, onStatusChange }) => {
  ...
  useEffect(() => {
    const fetchProductsForWarehouse = async () => {
      // Nếu là phiếu OUT (Xuất kho) thì filter theo kho xuất (fromLocationId)
      // Nếu là phiếu IN (Nhập kho) thì filter theo kho nhập (toLocationId)
      const locId = formData?.documentType === 'IN' ? formData?.toLocationId : formData?.fromLocationId;
  ...
  }, [formData?.fromLocationId, formData?.toLocationId, formData?.documentType, isEditing]);
  ...
  const handleStatusChange = async (newStatus: TransferStatus | 'READY') => {
    ...
    try {
      ...
      const message = (rawMessage || "Có lỗi xảy ra khi chuyển trạng thái").trim();
      if (message.includes("Tồn kho hiện tại không đủ")) {
        toast.info("Tồn kho hiện tại không đủ.", "Cảnh Báo");
      } else {
              ...
              <div className="shrink-0 flex items-center justify-between gap-4 mb-2">
                ...
                {isEditing && (
                  <button
                    type="button"
                    onClick={addLine}
                    className="px-4 h-9 bg-[#E4002B] text-white text-[9px] font-black uppercase tracking-[0.08em] rounded-full border border-[#E4002B] hover:bg-[#cc0027] active:scale-95 transition-all whitespace-nowrap shrink-0"
                  >
                    + THÊM SẢN PHẨM
                  </button>
                )}
                      ...
                      <div className="min-w-0">
                        ...
                        >
                          <DatePicker
                            ...
                            className={`box-border w-full min-h-0 flex-1 bg-white border border-gray-200 rounded-xl px-3.5 pr-16 text-[10px] font-bold leading-none text-gray-800 tabular-nums outline-none transition-all kfc-form-field-shadow placeholder:font-medium placeholder:text-gray-400 ${isEditing ? 'focus:border-[#E4002B] cursor-pointer' : 'cursor-default'}`}
                          />
                          <button
                            type="button"
                            className={`absolute right-11 top-1/2 z-[2] -translate-y-1/2 rounded-md p-1 text-gray-400 transition-colors ${isEditing ? 'hover:bg-gray-100 hover:text-gray-600' : 'opacity-0 pointer-events-none'}`}
                            aria-label="Mở lịch"
                            onMouseDown={(e) => e.preventDefault()}
                            onClick={() => {
                              const el = document.getElementById('transfer-detail-date-mgr');
                              el?.focus();
                              el?.click();
                            }}
                          >
