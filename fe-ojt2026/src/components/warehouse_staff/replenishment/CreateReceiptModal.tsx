...
export default function CreateReceiptModal({ isOpen, receiptData, onClose, onSave }: any) {
    ...
    if (receiptData) {
      ...
      const mappedItems = (receiptData.items || []).map((it: any) => {
        ...
        return {
          ...
          actual_qty: isReceiving ? remaining : 0
        };
      });
      ...
      prevItems.map(item => {
        if (item.id === id) {
          ...
          if (field === 'quantity') {
            ...
            // Tự động cập nhật số lượng trong lô nếu chỉ có 1 lô (trường hợp Draft)
            const updatedLots = (item.lots || []).map((lot: any, idx: number) =>
              idx === 0 ? { ...lot, quantity: numVal } : lot
            );
            return {
              ...item,
              quantity: numVal,
              lots: updatedLots,
              subtotal: numVal * (item.unitPrice || 0)
            };
          }
          if (field === 'actual_qty') {
            const numVal = value === "" ? 0 : Number(value);
            // Tự động cập nhật số lượng thực nhận trong lô nếu chỉ có 1 lô (trường hợp Confirm/Partial)
            const updatedLots = (item.lots || []).map((lot: any, idx: number) =>
              idx === 0 ? { ...lot, quantity: numVal } : lot
            );
            return {
  ...
  const handleSaveLots = (updatedLots: any[]) => {
    if (activeLotItem) {
      ...
      if (isReadyMode) {
        updateItemRow(activeLotItem.id, 'actual_qty', totalQty);
      } else {
        updateItemRow(activeLotItem.id, 'quantity', totalQty);
      }

      setError(null); // Xóa lỗi ngay khi lưu lô thành công
  ...
  const handleRemoveItem = (id: number) => {
    // Nếu là item đã tồn tại trên DB (id > 0 và < 1 tỷ), ta cần lưu lại để xóa khi Save
  ...
  const handleSaveDraft = async () => {
    if (!vendorId || !toLocationId) {
      setError("Vui lòng chọn Nhà cung cấp và Vị trí từ danh sách!");
      return;
    }
    ...
    try {
      ...
      // Xóa các item đã bị remove khỏi UI trong quá trình Edit
      if (deletedItemIds.length > 0) {
        console.log(">>> [API DELETE ITEMS] Xóa các item IDs:", deletedItemIds);
        await Promise.all(
          deletedItemIds.map(id => purchaseOrderApi.deletePurchaseOrderItem(id))
        );
        setDeletedItemIds([]); // Reset sau khi xóa thành công
      }
      ...
      notifyFormError(`Không thể lưu nháp. ${typeof msg === "string" ? msg : "Liên hệ Admin"}`);
    } finally { setIsSavingDraft(false); }
  };

  const handleConfirmOrder = async () => {
    if (!vendorId || !toLocationId) { notifyFormError("Vui lòng chọn NCC và Vị trí!"); return; }
    const apiItems = prepareItemsForApi(items, false);
    if (apiItems.length === 0) { notifyFormError("Vui lòng thêm sản phẩm!"); return; }
    ...
    try {
      ...
      const res: any = await purchaseOrderApi.confirmPurchaseOrder(body);
      const serverPoId = res.Id || res.id || poId;
      const serverDocId = res.DocId || res.docId || docId;
      ...
      notifyFormError("Backend từ chối xác nhận (có thể do thiếu số lượng)");
    } finally { setIsConfirming(false); }
  };

  const handleCompleteOrder = async () => {
    const invalidItem = items.find((it: any) => (Number(it.actual_qty) || 0) > (Number(it.quantity) - Number(it.receivedQty || 0)));
    if (invalidItem) { notifyFormError(`Vượt quá số lượng còn lại!`); return; }
    ...
    try {
      ...
      const res: any = await purchaseOrderApi.completePurchaseOrder(body);
      const serverPoId = res.Id || res.id || poId;
      const serverDocId = res.DocId || res.docId || docId;
      ...
      notifyFormError("Lỗi hoàn thành");
    } finally { setIsConfirming(false); }
  };
  ...
  const fetchReceptionHistory = async () => {
    ...
    try {
      ...
      console.log(">>> [STAFF HISTORY] Raw Transactions:", transactions);
      ...
      setError("Không thể tải lịch sử nhận hàng");
    } finally {
    ...
    <>
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center p-2 sm:p-3 animate-in zoom-in-95 duration-200 pointer-events-none overflow-y-auto overscroll-contain">
        <div className="bg-white rounded-[2rem] border border-[#E4002B]/30 shadow-sm w-[1200px] max-w-[96vw] h-[min(86vh,calc(100vh-1rem))] max-h-[calc(100vh-1rem)] min-h-0 flex flex-col pointer-events-auto overflow-hidden animate-in fade-in duration-300 my-auto">
          ...
          <div className="flex-1 min-h-0 px-4 py-2 sm:px-5 overflow-hidden bg-white flex flex-col gap-1.5">
            {error && (
              <div className="shrink-0 p-4 bg-red-500/5 rounded-[1.5rem] border border-red-500/10">
            ...
            <div className="flex-1 min-h-0 grid min-w-0 grid-cols-1 min-[1000px]:grid-cols-[minmax(0,1fr)_minmax(0,438px)] gap-4 overflow-hidden">
              <div className="min-h-0 min-w-0 rounded-[1.5rem] border border-gray-100 bg-white p-3 overflow-hidden flex flex-col shadow-md shadow-gray-200/40">
                ...
                <div className="flex-1 min-h-0">
                  <ReceiptItemsTable
                    items={items}
                    status={(isEditMode || !isDraftStatus) ? status : 'Completed'}
                    masterIngredients={masterIngredients}
                    onUpdateItem={updateItemRow}
                    onAddItem={handleAddProductLine}
                    onRemoveItem={handleRemoveItem}
                    onOpenLotModal={(item: any) => {
                      // Cho phép mở Lot Modal nếu là Sẵn sàng (không cần Edit mode) hoặc nếu đang Edit mode cho Draft
              ...
              <div className="flex h-full min-h-0 min-w-0 flex-col gap-2">
                <div className="flex-1 min-h-0 min-w-0 overflow-hidden px-0.5 pr-2.5 pb-1.5 pt-0.5">
                  <div className="min-h-0 max-h-full overflow-y-auto custom-scrollbar">
                    <ReceiptHeader
                      vendor={vendor}
                    ...
                    <div className="shrink-0 border-b border-white/20 pb-1.5 flex items-start justify-between gap-2">
                      ...
                      <p className="text-[10px] font-black text-white tabular-nums whitespace-nowrap uppercase tracking-[0.08em]">
                        Ngày tạo {(() => {
                          ...
                          const dt = creationDate.includes("-") ? new Date(`${creationDate}T12:00:00`) : new Date(creationDate);
                    ...
                    <div className="flex flex-col gap-1 text-[9px] font-bold leading-tight shrink-0">
                      ...
                      <div className="flex gap-2 items-baseline min-w-0">
                        ...
                        <div className="flex flex-1 min-w-0 justify-between gap-2 items-baseline">
                          ...
                          <span className="text-[12px] font-black whitespace-nowrap overflow-hidden text-ellipsis max-w-[190px] text-right text-white" title={`${new Intl.NumberFormat("vi-VN").format(totalAmount)} VND`}>
      ...
      {activeLotItem && (
        <LotDetailModal
          ...
          isReadOnly={!(String(status).toLowerCase().includes("draft") || status === "Dự thảo" || status === "Nháp")}
