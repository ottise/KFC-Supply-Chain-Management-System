...
export default function StockPagination({
  ...
  const hasNext = hasNextProp ?? safeCurrentPage < totalPages;
  const hasPrevious = hasPreviousProp ?? safeCurrentPage > 1;
    ...
    <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-gray-100">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          disabled={totalPages === 0 || safeCurrentPage === 1 || !hasPrevious}
        ...
        <button
          onClick={() => onPageChange(safeCurrentPage + 1)}
          disabled={safeCurrentPage === totalPages || totalPages === 0 || !hasNext}
          className="px-3 py-2 rounded-lg text-xs font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100 text-gray-600"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
