...
import type { ProductWarehouseItem } from "@/lib/api/warehouse/productWarehouseApi";
...
const CreateTransferForm: React.FC<Props> = ({ onClose, onSave }) => {
  ...
  const notifyFormError = useCallback((detail: string) => toast.error("Thông tin chưa hợp lệ", detail), [toast]);
  const notifyActionError = useCallback((detail: string) => toast.error("Không thể thực hiện thao tác", detail), [toast]);
  const notifySuccess = useCallback((detail: string) => toast.success("Thao tác thành công", detail), [toast]);
  ...
  useEffect(() => {
    ...
    fetchInitialData();
  }, [user?.id, user?.managerId, notifyActionError]);
  ...
  useEffect(() => {
    ...
    const fetchTransferableProducts = async () => {
      try {
        ...
        const registeredIds = new Set<number>((toWarehouseRes?.Items || []).map((it: ProductWarehouseItem) => Number(it.ProductId || it.Id)));
        const transferableProducts = (fromProducts || []).filter((p: Product) => registeredIds.has(Number(p.Id)));
        ...
        setItems(prev => prev.map(item => {
          ...
          if (currentProductId && !transferableProducts.some((p: Product) => Number(p.Id) === Number(currentProductId))) {
  ...
  }, [formData.fromLocationId, formData.toLocationId, locations, notifyActionError, notifyFormError]);
  ...
  useEffect(() => {
    const loadWarehouses = async () => {
      try {
        ...
        const list = (Array.isArray(res) ? res : (res?.Items || res?.items || [])) as Warehouse[];
        const map: Record<number, string> = {};
        list.forEach((w: Warehouse) => {
          const id = Number(w.Id);
          if (id) map[id] = String(w.Name || "");
        });
  ...
  const updateItem = useCallback((id: number, field: keyof TransferItem, value: string | number | null) => {
    setItems(prev => prev.map(item => {
      if (item.id === id) {
  ...
  }, [formData.fromLocationId, formData.toLocationId, products, toWarehouseRegisteredProductIds, notifyFormError]);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const backendMsg = err.response?.data?.message || err.response?.data?.title || err.message;
          ...
          <div className="flex flex-col items-center text-center min-w-0">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <h2 className="text-lg md:text-xl font-black text-white uppercase tracking-tight mt-1.5 leading-tight">Lập phiếu điều chuyển nội bộ</h2>
              </div>
            </div>
            <p className="text-[10px] font-black text-white/80 uppercase tracking-widest mt-1">Luân chuyển hàng hóa giữa các kho trong hệ thống</p>
          </div>
